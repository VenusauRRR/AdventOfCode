...
int main(void)
{
    ...
    std::vector<long long> freshList;
    int sum = 0;
    ...
    {
        for (long long i = it.first; i <= it.second; i++)
        {
            if (freshList.size() != 0)
            {
                auto temp = std::find_if(freshList.begin(), freshList.end(), [i](long long &nr)
                                         { return nr == i; });

                if (temp == freshList.end())
                {
                    freshList.push_back(i);
                }
            }
            else
            {
    ...
    long freshListSize = freshList.size();
    std::cout << "fresh List Size: " << freshListSize << std::endl;
    // for (auto it : rangeList)
    // {
    //     std::cout << "(" << it.first << ", " << it.second << ")" << " ";
    // }
